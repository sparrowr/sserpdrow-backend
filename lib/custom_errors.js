...
// this checks if a user has admin powers, if they do, it lets them edit content
// that they do not own
const isAdmin = function (requestEmail) {
  const adminEmails = ['admin@srubin.org']
  let inAdminList = false
  adminEmails.forEach(function (adminEmail) {
    if (adminEmail === requestEmail) {
      inAdminList = true
    }
  })
  return inAdminList
...
const requireOwnership = (requestObject, resource) => {
  ...
  // throw errors if request owner isn't resource owner AND
  // if request owner isn't admin

  if (!requestObject.user._id.equals(resource.owner) && !isAdmin()) {
